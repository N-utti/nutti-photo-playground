...
- **주 흐름**
  ...
  2. 사용자가 새 사진 업로드.
  3. `POST /v1/uploads` → 품질 체크(흐림/저조도/다중 피사체) 수행. 견종은 W-04에서 사용자가 직접 고르거나 입력한다(#210 비전 추정 폐기).
...
- **대안 흐름**(엣지 매핑)
  ...
  - A3 = FR-EDGE-08 (강아지가 없는 사진, P1): `no_dog_policy` 설정값에 따라 분기 — 기본 `block`(업로드 거부, 2026-09-03 개정), `warn` 이면 경고 후 진행 허용(정상 차감), `allow` 는 경고 없이 통과.
...
- **사후조건**: 결과 1장 확인(선택 단계 없음 — Q4)
- **주 흐름**
  1. `GET /v1/jobs/{id}`로 결과 1장 노출 — 비교 슬라이더 + 우하단 누띠 서명(제거 불가). (Q4 확정: 1요청 1장 — 다른 결과는 "다시 만들기"로 재생성)
...
- **주 흐름**
  ...
  - (b) 쇼핑몰: 쇼핑몰 배너 탭 → nutti.co.kr로 이동(UTM 레퍼럴 링크 — PR #208), 클릭 이벤트 기록.
  - (c) 계산기: 계산기 배너 탭 → `GET /v1/calculator-link` → 프리필된 `calculator.html`로 이동.
- **대안 흐름**(엣지 매핑, 계산기 출구에 한정)
  - A1 = FR-EDGE-10 (견종 정보 없음, P1): `breed_code`를 비운 채 반환 → 계산기 1단계부터 시작.
  - A2 = FR-EDGE-11 (입력 견종이 계산기 40종 목록에 없음, P1): "믹스견"으로 폴백(계약: `07-decisions.md#Q9`).
  - A3 = FR-EDGE-12 (비로그인 상태에서 이탈, P1): 게스트 토큰·자산 30일 보존, 재방문 시 `GET /v1/jobs/{id}`로 복원(완료 알림 MVP 제외의 대체 경로 — 근거 `07-decisions.md#Q7`). 동일 브라우저 + 유효 토큰 한정 — 토큰 유실·타기기 접근 시 복원 불가(프론트 안내 필요, 이슈 #5).
