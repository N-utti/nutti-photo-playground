...
| `source_image` (게스트) | `member.guest_expires_at`(30일) 기준 — 별도 `expires_at` 컬럼 없음(2026-09-14 삭제) | 만료 시 정리 배치 대상(회원 승격 시 이관되어 해당 없음) |
| `source_image` (회원) | 무기한 | 보관함(W-09)에 남아있는 한 보존, 삭제는 사용자 요청 시에만 |
