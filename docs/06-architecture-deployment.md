...
- 워커가 job을 집으면 `status='processing'`, `lease_expires_at = now() + interval '360 seconds'`, `attempt_count += 1`로 갱신합니다. 리스는 생성 타임아웃(300s)보다 길어야 합니다 — 짧으면 §2.5 의 동시 슬롯이 아직 도는 job 을 「만료」로 다시 집어 두 번 생성합니다(2026-09-03, 90→360 상향). 대가는 워커 사망 시 회수까지 6분.
...
**Q4 확정(2026-08-04, PO): 1요청 = 1장.** 워커는 job당 프로바이더 요청 1건만 발행합니다 — 원가가 4장 대비 1/4로 줄고, 다른 결과가 필요하면 사용자가 "다시 만들기"(새 job·새 크레딧)로 재생성합니다. `style.output_count`(기본 1)·`generation_result.seq` 구조는 유지하므로, 산출 수를 상향할 경우 `asyncio.gather` 동시 발행으로 확장하면 됩니다(I/O 바운드라 소요 시간은 "가장 느린 1장" 수준에 수렴).
...
### 2.5 확장 계획 — 생성 처리량·메모리 (2026-09-03 실측 기준)

**현재 상태(2GB VPS 1대, vc2-2c-2gb)**

| 컨테이너 | RSS | 비고 |
|---|---|---|
| api (uvicorn 1 프로세스, async) | 160 MB | 업로드 순간에만 요청당 ~30 MB 추가(파일 10 MB 상한 + 리사이즈 + 비전용 base64) |
| worker | 108 MB | `WORKER_CONCURRENCY`=4 슬롯 동시 생성. 슬롯당 이미지 1장 분량(수 MB) |
| postgres | 35 MB | 데이터 65 MB |
| caddy | 24 MB | |

스왑 6.4 GB, 가용 메모리 ~1.15 GB(캐시 포함). 생성 1장 = fal 응답 대기 ~50초(p95 59초)라 **I/O 바운드** — 병목은 메모리가 아니라 **동시 처리 수**다. 슬롯 4 기준 시간당 약 280장, 동시 요청 4명까지는 전원 1분 안, 8명이면 마지막 사람이 2분.

**언제 무엇을 올리는가 — 순서대로, 앞 단계가 부족할 때만 다음**

| 단계 | 신호(관리자 콘솔 「로그」탭 · 헬스 SQL) | 조치 | 비용·부작용 |
|---|---|---|---|
| 0 | 없음 | 아무것도 안 함 | — |
| 1 | `queued` 잡이 5분 넘게 남거나, 동시 요청이 슬롯 수를 자주 넘음 | 서버 `.env` `WORKER_CONCURRENCY=8`(→12) 후 `up -d --no-deps worker` | 코드 변경 0, 메모리 +수십 MB. 상한은 fal 큐·계정 동시 한도 |
| 2 | 슬롯 12에서도 대기열이 쌓임 · 워커 1개의 CPU(Pillow 서명·JPEG 인코딩)가 100% 근처 | 워커 컨테이너 2~3개(compose `worker` 서비스 복제 또는 `--scale worker=3`). 폴링 쿼리가 `SKIP LOCKED` 라 조율 불필요 | 개당 ~100 MB. 2 GB 서버에서 3개까지 |
| 3 | api+worker RSS 합 > 1.2 GB 지속 · 스왑 사용 증가 · 동시 업로드 폭주 | VPS 4 GB(vc2-2c-4gb)로 리사이즈. 도커 볼륨 그대로, 다운타임 수 분 | 월 요금 2배. **postgres 는 이때도 같은 박스** |
| 4 | §2.4 arq 승격 임계 중 하나 충족(재처리 1%↑ · 폴링 P95 10초↑ · 다중 서버) | Redis + arq, 워커를 별도 서버로 | 컴포넌트 +2, 운영 부담 상승 |
| 5 | api 자체가 CPU 포화(업로드 리사이즈·비전 인코딩) | uvicorn `--workers 2`(2코어) → 그래도 부족하면 api 서버 분리 | 세션은 JWT 라 무상태, 분리 쉬움 |

**같이 챙길 것**

- 배포마다 api 재시작 ~5초 502 → 트래픽이 늘면 compose healthcheck + `start_first` 식 롤링(또는 caddy 업스트림 2개)로 무중단화.
- 헬스 신호를 사람이 보지 않아도 되게: 「queued > 5분」「메모리 가용 < 300 MB」「실패-미환불 > 0」 을 5분 주기로 재고 슬랙 알림(§9 `ADMIN_ALERT_SLACK_WEBHOOK_URL`). 임계 넘으면 1단계부터.
- fal 쪽 한도(계정 동시 요청·잔액 403 플래핑)가 먼저 걸릴 수 있다 — 슬롯을 올릴 때 fal 대시보드 동시 한도도 같이 확인.
- 비용은 처리량과 무관하게 **장당** $0.053(medium)이다. 확장이 원가를 바꾸지 않는다.

---

## 3. AI 프로바이더 비교 (Q4 결정 전제 데이터)

Q4("1회 생성 산출 수와 원가")의 판단 근거였으며, **1장으로 확정**되었습니다(§2.3). "내 강아지 특징 보존"은 리서치 인사이트6(`wireframe-spec-v0.5.html#p00`)이 요구하는 핵심 품질 기준입니다.
...
**Q4 원가 계산**(1장/job 확정 기준): Nano Banana $0.039/job(약 55원), Flux Kontext Pro $0.04 + 입력 이미지 비용 ≈ $0.045~0.05/job. 4장 대비 1/4로 절감 — 무료 체험 남용 제한 강도(§3.4, 03-usecases 참고)는 이 단가에 연동해 결정합니다.
...
3. 워커가 SKIP LOCKED로 job을 lease(§2.2) → 원본을 R2에서 읽어 AI 프로바이더에 요청 1건 발행(§2.3).
4. 프로바이더 응답 이미지를 워커가 받아 **서버 사이드에서 누띠 서명을 합성**(예: Pillow로 로고+텍스트 오버레이 — 클라이언트가 합성하면 서명 없는 원본이 네트워크에 노출되고 변조도 쉬워짐).
5. 합성된 결과 1장을 R2에 업로드(`storage_key` UUID), `generation_result` row 1개 INSERT.
...
- job당 R2 업로드량: 결과 1장(서명 합성 JPEG, ~800KB) + 원본 1장(~1.2MB) ≈ **2MB/job**
...
| `WORKER_CONCURRENCY` | 워커 동시 생성 수(기본 4). 생성은 fal 대기라 한 프로세스가 겹쳐 돌린다 — 대기열이 쌓이면 올리고, 그래도 부족하면 워커 컨테이너를 늘린다(SKIP LOCKED) | 4 |
