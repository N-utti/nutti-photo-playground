...
**회원 세션 = 액세스(1h) + 리프레시(30일, 회전)** — 이슈 #47(2026-08-10 확정). MemberSession을 반환하는 모든 응답(소셜 콜백·register·login)에 `refresh_token`이 포함되며 원문은 이때 한 번만 노출됩니다(서버는 해시만 보관, 회원당 활성 1개 — 새 로그인 시 이전 세션 무효화). 게스트는 리프레시 없음(30일 JWT + `POST /v1/auth/guest` 재발급 유지). 게스트 토큰은 JWT `exp` 와 별개로 `member.guest_expires_at`(자산 보존 기한, 발급·핸드오프 때 `JWT_GUEST_EXPIRES_IN` 만큼) 이 지나면 `401 TOKEN_EXPIRED` — purge 가 자산을 지운 뒤 토큰만 살아 있는 세션을 막는다(2026-09-14).
...
- 게스트가 소진하면 `guest_expires_at` 도 새 토큰 수명만큼 연장된다(2026-09-14 — 그전엔 토큰만 늘고 자산은 원래 기한에 purge 됐다).
...
> **연동 회원은 조회 시 그 회원의 주문 동기화를 백그라운드로 예약**한다(2026-09-02, #225 — 응답은 카페24를 기다리지 않는다). 주 지급 경로는 결제 웹훅(#201, 결제 즉시 +20)이고 이 동기화는 놓친 건 보정용: 카페24 `GET /admin/orders?member_id=`를 `order_reward_cutoff`부터 조회해 배치와 같은 규칙(`app/cafe24.py _apply_order`)으로 지급/회수. 이 조회는 거의 모든 화면의 앱바 크레딧 배지가 쓰므로 카페24가 느려져도 배지가 최대 30초를 기다리는 일이 없어야 한다. 회원당 **60초에 1회**(새로고침 연타 보호), 실패는 로그만(화면은 절대 안 죽음). 30분 크론은 보정용으로 유지. 미연동 회원은 카페24 호출 없음. 갱신된 잔액은 다음 조회(웹훅이 정상일 땐 이미 반영돼 있음)에 나타난다.
...
`key`는 서버가 아는 설정 키(`human_face_policy`, `no_dog_policy`, `custom_prompt_credit_cost`, `daily_free_amount`, `follow_ig_amount`, `link_account_amount`, `order_reward_amount`, `guest_trial_amount`(게스트 발급 시 체험 크레딧, 기본 1 — 2026-09-14), `catalog_search_threshold`)만 허용. GET은 저장된 적 없는 키도 기본값으로 돌려주며 그 경우 `updated_at`은 `null`. `400 VALIDATION_ERROR`: `human_face_policy`·`no_dog_policy`는 `block`/`warn`/`allow`, 나머지는 0 이상 정수만.
