...
5. Webhooks: 콜백 URL `https://api.nutti.co.kr/v1/webhooks/instagram`, 확인 토큰 = `.env` `INSTAGRAM_WEBHOOK_VERIFY_TOKEN`(임의 문자열) → 구독 필드 **`comments`, `messages`**. 콘솔 [테스트] 전송으로 api 로그 200 확인. 앱을 **Live 모드**로 전환(웹훅은 Live 에서만 온다 — 전환 자체엔 검수 불필요).
   - **검수 전**: `comments` 웹훅은 Advanced Access 가 있어야 와서 실제로는 안 온다. 대신 api 가 `INSTAGRAM_COMMENT_POLL_SECONDS`(기본 60초)마다 댓글이 달린 게시물(최대 200개 중)의 댓글을 읽어 같은 비공개 답장을 보낸다(내 계정 읽기·DM 은 Standard Access 로 충분). 켠 시점 이후 댓글만 본다(워터마크 `app_setting.instagram_comments_polled_at`). `messages` 웹훅은 검수 없이 온다.
