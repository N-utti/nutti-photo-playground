 ...
 * 하나를 놓고 각자 문장을 만드는데, 한쪽만 "입력한 견종 · 토이푸들" 이라고 하고 다른
 * 쪽이 "견종을 안 골랐어요" 라고 하면 같은 값을 두고 앱이 스스로를 반박합니다. 규칙을
...
describe('estimateSummary', () => {
  it('견종과 크기를 알면 입력한 값임을 밝히고 2단계부터 시작한다', () => {
    /*
      «입력한 견종» — 사용자가 W-04 에서 고른 값임을 밝힙니다. 이 출구의
    ...
    */
    expect(estimateSummary({ breed_label: '토이푸들', size_label: '소형' })).toEqual({
      text: '입력한 견종 토이푸들 · 소형 · 2단계부터 시작',
      prefilled: true,
      kind: 'matched',
    })
  })

  it('믹스견이면 입력한 견종이라고 하지 않는다 (FR-EDGE-11)', () => {
    /*
      이 값은 **폴백일 수 있습니다** — 사용자가 «골든두들» 이라고 썼는데 계산기 40종에
      없어서 서버가 대신 넣은 값이면, "입력한 견종 믹스견" 은 거짓말입니다. 반대로
      진짜 믹스견을 골랐을 수도 있어 "목록에 없어서" 라고 단정하는 것도 틀립니다
      (응답만으로는 구분 불가). 둘 다에 대해 참인 말만 남깁니다.

      그래도 `prefilled` 는 참입니다 — 계산기는 실제로 2단계부터 시작합니다.
    */
    expect(estimateSummary({ breed_label: '믹스견', size_label: '중형' })).toEqual({
      text: '믹스견 · 중형으로 넘겨요 · 2단계부터 시작',
      prefilled: true,
      kind: 'mixed',
    })
  })
  ...
  it('견종을 모르면 1단계부터 시작한다', () => {
    ...
    */
    expect(estimateSummary({ breed_label: null, size_label: null })).toEqual({
      text: '견종을 입력하지 않았어요 · 1단계부터 시작',
      prefilled: false,
      kind: 'unknown',
    })
  })

  it('견종 라벨이 빈 문자열이어도 «입력하지 않았어요» 로 본다', () => {
    // 서버가 빈 문자열을 줄 때 «입력한 견종  · 소형» 같은 반쪽 문장이 되지 않아야 합니다.
