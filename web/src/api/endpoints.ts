...
export const auth = {
  ...
  /**
   * 회원이면 서버가 **리프레시 토큰을 폐기**합니다(PR #57) — 그래서 이 호출은 이제
   * 예의가 아니라 실제 효과가 있습니다. 액세스 토큰은 서명만으로 검증되므로 남은 수명
   * (최대 1h) 동안 유효한 창은 그대로 수용합니다(이슈 #11 L4).
   *
   * 그래도 실패를 삼키는 건 그대로입니다 — 서버에 못 닿았다고 로컬에 회원 토큰을
   * 남겨 두면 사용자는 로그아웃을 눌렀는데 계속 로그인 상태인 화면을 보게 됩니다.
   * 인증이 필요한 엔드포인트가 됐으므로(`get_current_member`) 토큰 없이 부르면 401 인데,
   * 그 401 도 여기서 끝납니다.
   */
  logout: async () => {
    try {
