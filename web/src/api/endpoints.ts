...
/**
 * `POST /v1/events` — **더는 501 스텁이 아닙니다**(백엔드 PR #177). 한 번 부를 때마다
 * `metric_event` 한 행이 실제로 쌓이고, 그게 W-11 운영 콘솔이 나눠 쓰는 분모입니다.
 * 501 이던 동안에는 «한 번 더 보내도 아무 일 없음» 이었지만 지금은 아닙니다.
 *
 * ## `properties.job_id` 는 자유 텍스트가 아닙니다
 *
 * 서버가 이 **키 이름을 직접 읽습니다**. 값이 호출자 본인 소유 job 이면 `metric_event`
 * 의 `job_id`·`style_id` 외래키로 붙고, 그게 **스타일별 성과 집계의 유일한 경로**입니다
 * (이벤트 자체에는 스타일이 없습니다). 파싱 실패·타인 소유·미존재는 에러가 아니라
 * 조용히 null 로 저장됩니다 — 비콘이 화면을 막지 않는다는 계약을 서버가 지켜 준
 * 결과지만, 대가로 **틀리게 보내도 아무도 알려 주지 않습니다.** 키를 `jobId` 로 고치거나
 * job 맥락에서 빼먹으면 요청은 그대로 204 이고 집계만 비어 갑니다.
 *
 * `properties.style_id` 를 직접 실어도 서버는 **아직 읽지 않습니다**(PR #177 코드 주석
 * — 프론트가 보내기 시작할 때 추가). 스타일을 붙이는 방법은 오늘도 `job_id` 하나뿐입니다.
 *
 * ## 삼키는 것은 예외지 부작용이 아닙니다
 *
 * 이제 **인증이 필수**라(게스트·회원 모두) 이 요청도 공용 클라이언트의 세션 경로를
 * 그대로 탑니다. `catch` 는 프라미스만 잠재웁니다 — 그전에 `request` 안에서 게스트
 * 재발급이나 `SESSION_LOST_EVENT` 가 이미 일어났을 수 있습니다. «비콘은 던지지 않는다»
 * 와 «비콘은 아무 일도 일으키지 않는다» 는 이제 다른 말입니다.
 */
export const events = {
