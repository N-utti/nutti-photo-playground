...
export default function W04Upload() {
  ...
  */
  ...
  function startGeneration() {
    ...
    /*
      이름이 인쇄되는 스타일(40 종 중 2 종)은 **이름 없이는 진행하지 않습니다**.
    ...
    <div className="screen-min-h bg-paper pb-16">
      <header className="sticky top-0 z-20 flex items-center gap-3 border-b border-rule bg-surface px-5 desktop:px-7 py-3 desktop:hidden">
        ...
        {/* 단계(선택 → 확인)에 따라 「사진 선택」「확인」으로 바뀌던 것을 화면 이름 하나로
            ...
            올린 순간 제목이 「확인」으로 바뀌면 다른 화면으로 옮겨진 것처럼 읽혔습니다. 단계는
            본문이 스스로 보여 줍니다(사진 · 경고 · 버튼). 이름은 라우트 제목(routes.tsx
            「사진 올리기」)과 같아 데스크톱 GNB 와도 일치합니다. */}
        <h1 className="text-base font-bold">사진 올리기</h1>
 ...
 * 40 종 중 이 스타일이 2 종뿐이라(3D_피규어 · 식빵, 실서버 실측) 나머지 38 종에는
