 ...
 *
 * ---------------------------------------------------------------------------
 * `uses_breed` 는 왜 **여전히** 이 화면에 없는가 (백엔드 #111 → #131 → PR #137)
 *
 * 예전 이유는 «한 번도 일어나지 않는 일이라서» 였습니다. 워커가 `[breed]` 를
 * `pet_profile.breed_label` 로만 채웠고 그 컬럼을 쓰는 API 가 없어서 늘 «강아지» 로
 * 떨어졌으며, 3D_피규어 프롬프트가 그 경우 라벨을 통째로 뺐습니다. **그 근거는
 * 2026-08-24 에 사라졌습니다** — 백엔드 #131 A안(PR #137)이 워커 폴백을 계산기와
 * 맞춰서(`breed_label` → `source_image.breed_estimate["label"]` → «강아지») 이제
 * 비전이 잡은 견종이 실제로 인쇄됩니다.
 *
 * 그런데도 여기 안 다는 이유는 **이 화면이 사진을 받기 전**이라서입니다. 무엇이
 * 인쇄될지는 스타일이 아니라 그 사진에서 견종이 잡혔는가로 갈립니다:
 *
 *   - `breed_label` 은 여전히 늘 NULL 입니다 — `POST /v1/pets`·`PATCH /v1/pets/{id}`
 *     가 `name` 만 받습니다(#131 의 B 안은 아직 안 왔습니다)
 *   - 그러니 값은 `breed_estimate["label"]` 하나로 결정되는데, 세 필드가 각각 null
 *     가능이라 사진마다 갈립니다(app/routers/uploads.py `BreedEstimate`)
 *   - 못 잡으면 «강아지» → 프롬프트가 라벨을 뺍니다 → 아무것도 안 찍힙니다
 *
 * 여기서 «견종도 함께 인쇄됩니다» 라고 쓰면 사진마다 참·거짓이 갈리는 약속이 됩니다.
 * `uses_pet_name` 을 여기 쓰는 건 이름이 **항상** 해결되기 때문이고(저장된 이름 아니면
 * «우리 아이»), 견종은 그 성질이 없습니다.
 *
 * 그래서 문구는 값을 손에 쥔 자리에 답니다 — **W-04 의 `BreedNotice`**
 * (screens/W04Upload.tsx). `POST /v1/uploads` 응답이 그 사진의 추정값을 주므로,
 * 거기서는 «이 사진에서 «토이푸들» 로 보여요» 라고 사실만 말할 수 있습니다.
 */
