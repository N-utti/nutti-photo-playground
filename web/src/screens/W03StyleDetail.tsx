 ...
 *
 * ---------------------------------------------------------------------------
 * `uses_breed` 는 왜 화면에 없는가 (백엔드 #111 착지분, W-02 도 같은 판단)
 *
 * 두 플래그는 계약상 짝이지만 **지금 서버에서 결과가 다릅니다**. `uses_pet_name` 은
 * 실제로 이름이 박히고, `uses_breed` 는 박히는 견종이 **없습니다** — 세 줄을 이으면
 * 그렇습니다:
 *
 *   1. 워커는 `[breed]` 를 `pet_profile.breed_label` 로 채우고, 없으면 «강아지»
 *      (app/worker.py) — 비전 추정(`source_image.breed_estimate`)은 안 봅니다.
 *      계산기 링크(app/routers/results.py)는 그 폴백을 타는데 워커만 안 탑니다.
 *   2. `breed_label` 을 쓰는 API 경로가 없습니다. `POST /v1/pets` 와
 *      `PATCH /v1/pets/{id}` 는 `name` 만 받습니다 — 즉 이 컬럼은 늘 NULL 입니다.
 *   3. 그래서 `[breed]` 는 항상 «강아지» 가 되고, 유일한 `uses_breed` 스타일인
 *      3D_피규어의 프롬프트 원문이 «강아지면 견종 라벨을 통째로 빼라» 고 지시합니다
 *      (seeds/prompts/3D_피규어.txt).
 *
 * 그러니 지금 «견종도 함께 인쇄됩니다» 라고 쓰면 한 번도 일어나지 않는 일을 예고하는
 * 것이고, 사용자는 «토이푸들» 을 기대하고 크레딧을 씁니다. 플래그는 받아 두되(계약이
 * 사실인 건 맞습니다) 화면 문구는 1·2 가 메워진 뒤에 답니다 — 백엔드 이슈 #131.
 * 그때 붙일 자리는 아래 이름 예고 문단 바로 옆입니다.
 */
