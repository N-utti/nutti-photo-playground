 ...
 *
 * ---------------------------------------------------------------------------
 * `uses_breed` 는 왜 **이 화면에** 없는가 (백엔드 #111 · #131, W-02 도 같은 판단)
 *
 * 한동안은 «인쇄된 적이 없어서» 였습니다. 워커가 `[breed]` 를 `pet_profile.breed_label`
 * 로만 채우는데 그 컬럼을 쓰는 API 경로가 없어 늘 NULL → «강아지» → 3D_피규어
 * 프롬프트가 «강아지면 견종 라벨을 통째로 빼라» 고 지시했습니다. 이유가 셋이었지만
 * 결론은 하나였습니다: 한 번도 일어나지 않는 일을 예고하면 안 된다.
 *
 * **그 전제는 사라졌습니다**(#131 A안 — `app/worker.py` 가 `breed_label` →
 * `source_image.breed_estimate["label"]` → «강아지» 로 한 단계 더 내려갑니다).
 * 이제 견종은 실제로 인쇄됩니다.
 *
 * 그래도 이 화면에는 안 답니다. 이유가 바뀌었습니다 — «일어나지 않아서» 가 아니라
 * **여기서는 무엇이 박힐지 모르기 때문**입니다. 견종은 사용자가 넣는 값이 아니라 그
 * 사진에서 추정한 값이고, 비전이 확신 못 하면 빈 채로 옵니다(PR #59). W-02·W-03 은
 * 업로드 **전**이라 아직 사진이 없습니다. 여기서 «견종도 함께 인쇄됩니다» 라고 쓰면
 * 보장할 수 없는 것을 약속하는 셈이고, 사용자는 «토이푸들» 을 기대하고 크레딧을 씁니다.
 *
 * `uses_pet_name` 과 갈리는 지점이 정확히 여기입니다. 이름은 우리가 받아서 넣으므로
 * 예고가 곧 약속이지만, 견종은 아닙니다. 그래서 견종을 말하는 자리는 **사진이 손에
 * 들어온 뒤**인 W-04 확인 단계 하나뿐입니다(`screens/W04Upload.tsx` BreedPrintNotice —
 * 추정값이 실제로 있을 때만, 출처를 밝히고 말합니다).
 */
