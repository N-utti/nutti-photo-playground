 ...
 *
 * 나머지 여덟 화면의 앱바(← + 제목)는 데스크톱에서 **붙어 서지 않습니다**
 * (`desktop:static desktop:border-0 desktop:bg-transparent`). GNB 가 이미 붙어 있는데
 * 그 밑에 흰 띠가 하나 더 붙으면 헤더가 두 줄로 읽혔고, 본문은 가운데 448px 인데 띠는
 * 화면 폭 전체라 «← 크레딧 받기» 두 어절이 1920px 흰 줄 왼쪽 끝에 떠 있었습니다.
 * 그래서 데스크톱에서는 페이지 바탕 위의 제목 한 줄로 내려놓고, 스크롤하면 같이
 * 올라가게 둡니다 — 붙여 두면 바탕색 띠 밑으로 본문이 지나가는 «떠 있는 크림 띠» 가
 * 다시 생깁니다(위 배경 주석과 같은 이유). 모바일은 GNB 가 없어 그대로 붙어 섭니다.
