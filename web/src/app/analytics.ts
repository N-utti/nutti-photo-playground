 ...
 * 건 어떤 지표보다도 비쌉니다.
 *
 * 다만 «막지 않는다» 가 «흔적이 없다» 는 아닙니다. 백엔드 PR #177 로 `POST /v1/events`
 * 는 인증이 필요한 진짜 쓰기가 됐고, 그래서 이 한 줄이 공용 클라이언트의 세션 경로를
 * 탑니다 — 만료된 게스트 토큰으로 비콘 하나를 쏘면 그게 재발급을 트리거합니다.
 * 자세한 건 `api/endpoints.ts` 의 `events` 주석.
 *
