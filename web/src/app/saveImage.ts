 ...
 *
 * **이 함수를 `<a download>` 한 줄로 되돌리면 안 됩니다.** CORS 는 위의 fetch 를 허용할
 * 뿐, 교차 출처에서 무시되는 `download` 속성을 되살리지 않습니다(브라우저가 속성 자체를
 * 없는 것으로 칩니다). 되돌리면 #77 이 고치려던 버그가 그대로 재발합니다.
 *
 * **다만 이제 이 경로가 «유일한 길» 은 아닙니다.** 아래 `downloadAttachment` 가 서버의
 * 첨부 주소(`download_url`)로 이동해 저장하는데, 그건 `download` 속성이 아니라 응답의
 * `Content-Disposition` 으로 저장되므로 교차 출처에서도 성립합니다(PR #252). 저장의 주
 * 경로는 그쪽이고 **이 blob 경로는 폴백**입니다 — 웹뷰가 아니고 OS 공유 시트도 없는
 * 브라우저가 여기로 옵니다. 둘의 차이는 「누가 파일임을 선언하나」입니다: 이쪽은
 * 클라이언트가 `blob:` 으로 만들어 선언하고, 저쪽은 서버가 헤더로 선언합니다.
 *
 * 그래서 두 경로 중 하나를 지우려 들지 마세요. 이쪽만 남기면 `blob:` 을 버리는 카톡·
 * 인스타 Android 웹뷰에서 저장이 조용히 실패하고(#252 가 고친 것), 저쪽만 남기면 서버가
 * 첨부 주소를 못 주는 경우에 물러설 곳이 없습니다.
 *
