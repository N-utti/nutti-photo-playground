...
| 운영 콘솔 | `/admin/*` | 구현 (PR #181~#187 — 08-28~31). 화면도 **생겼습니다** — 다만 백엔드가 올린 스탑갭이고 다른 오리진입니다(`api.nutti.co.kr/v1/admin/ui`, PR #229). 정식 W-11 은 여전히 프론트 몫 — 아래 W-11 절 |
...
src/
  ...
  app/
    routes.tsx      11개 화면 라우트 테이블 (W-03 은 W-02 의 자식 = 시트).
                    랜딩만 정적 import, 나머지는 `lazy` 로 청크 분리 + `warmScreens()`
...
### 첫 로드는 두 가지 규칙 위에 있습니다 (2026-09-02)

이 앱은 쇼핑몰의 홍보 유입구라 **첫 화면이 곧 이탈률**입니다. 그 자리를 지키는 규칙이
둘인데, 둘 다 지워도 화면은 멀쩡히 떠서 조용히 되돌아갑니다.

**1. 랜딩만 정적, 나머지 열한 화면은 청크로 갈라져 있습니다** (`app/routes.tsx` 의
`screen()`). 안 그러면 랜딩 한 장을 그리려고 결과(W-06)·업로드(W-04)·보관함(W-09)까지
전부 받습니다 — 그 셋을 랜딩에서 볼 확률은 0 입니다. 실측 **473.9 kB → 358.9 kB**
(gzip 139.7 → 115.7). 새 화면을 추가할 때 `element:` 로 쓰면 그만큼 첫 화면이 다시
무거워집니다. `React.lazy` 가 아니라 라우터의 `lazy` 인 이유는 파일 주석에 있습니다.

가른 대가로 «탭을 눌렀는데 청크를 기다리는 침묵»이 생기므로, 렌더가 끝난 뒤
유휴 시점에 `warmScreens()` 가 전부 미리 받습니다(gzip 45KB 남짓, 데이터 절약 모드는
존중). 이건 첫 페인트 **뒤**에 도는 것이 계약입니다 — 앞으로 당기면 가른 의미가 없습니다.

**2. 렌더는 게스트 발급을 기다리지 않습니다** (`main.tsx` → `openWhenSessionReady()`).
예전에는 `await ensureSession()` 뒤에 렌더했고, 그 왕복 내내 화면이 비어 있었습니다.
지금은 렌더가 먼저 나가고 **토큰이 필요한 요청**만 관문 앞에서 기다립니다(`api/client.ts`).
401 을 막는 효과는 그대로입니다. 실측 **FCP 248ms → 148ms**, 목 위에서 잰 값이라
실서버 모바일 회선에서는 차이가 더 큽니다.

`request()` 의 관문 한 줄을 지우면 첫 방문자만 401 을 받습니다 — 목에서는 발급이
빨라 거의 재현되지 않습니다. 그래서 화면이 아니라 **요청이 나간 시점**을
`api/client.test.ts` 의 「부팅 관문」이 셉니다.
...
예전 성공 카드는 실패 화면과 **완전히 같은 껍데기**라 로그인에 성공한 사람이 «에러인가?»
하고 멈췄고, 그 뒤를 이은 「로그인됐어요」 모달은 사용자가 연 적 없는 창이 하던 일 위에
얹히는 것이라 둘 다 걷어냈습니다. 이메일 로그인 시트도 성공하면 그냥 닫힙니다 — 돌아간
화면의 계정 자리와 크레딧 배지가 이미 로그인됐음을 말하고 있습니다.
...
| Phase 5 | W-08 크리에이티브 · W-11 운영 콘솔(번들 분리) | W-08 완료 · W-11 미착수 — **백엔드는 전부 착지**(PR #181~#187)했고 **스탑갭 콘솔까지 올라왔습니다**(PR #229, 다른 오리진). 남은 공백은 FR-W11-03·06·08, 아래 절 |
...
4번의 키는 `VITE_KAKAO_JS_KEY` 로 `.env.production` 에만 있습니다(공개값 — 보호는 위
도메인 등록이 합니다). `.env.development` 는 비워 두고, 비면 `kakaoShareAvailable()` 이
false 라 항목 자체를 안 그립니다. 2026-09-03 등록 시 `http://localhost:5190` 도 같이
...
## 계산기 프리필 **수신부** — 구현 완료 (2026-09-02)

2026-08-28 실측 때는 계산기가 URL 파라미터를 읽지 않아(«2단계부터 시작» 이 참이 아니던 상태)
배포 직전 재실측 항목이었는데, **2026-09-02 스마트디자인 `calculator.js` v=15 에 수신부가
들어갔습니다**: `?breed=` 가 40종 한글명과 매칭되면 카드 선택 + 2단계 진입, `?name=` 은 이름칸
주입, 미매칭·미지정은 조용히 1단계 유지(size 는 견종에서 나오므로 읽지 않음). 실측:
`?name=콩이&breed=토이푸들` → 2단계·이름 프리필·`state.breed`=토이푸들 확인. 따라서
`api/calculatorLink.ts` 의 «2단계부터 시작» 문구는 이제 사실입니다. 계산기 목록이 바뀌면
`app/breeds.py`·`api/breeds.ts` 와 함께 수신부 매칭도 같은 한글명 계약을 유지해야 합니다.
...
운영 설정(#186) · 카페24 상태(#187). **오늘 프론트가 고칠 코드는 없습니다** — 이 저장소에
W-11 화면이 없으니 부르는 자리도 없습니다. 다만 착수 전에 알아야 하는 것이 있습니다.

### 0. 이제 화면이 **있습니다** — 다른 오리진에 (2026-09-02, 백엔드 PR #229)

`GET /v1/admin/ui` 가 `app/static/admin.html` 단일 파일을 내려 줍니다. 「W-11 정식 화면이
오면 이 파일과 라우트만 지운다」고 파일 첫 줄에 적혀 있습니다 — **백엔드는 W-11 을 여전히
프론트 몫으로 두고 있고, 대체를 선언하지 않았습니다.**

부르는 API 는 제가 만들 것과 **100% 같습니다**(`/v1/admin/*` 전부). 처음엔 비인증 정적
서빙 라우트 하나만 늘었지만, 그 뒤 **조회 API 4개가 콘솔 몫으로 생겼습니다**(백엔드 PR #264):
회원 목록(`/members`) · 잡 로그(`/jobs`) · 행동 이벤트(`/events`) · 크레딧 원장(`/ledger`).
`docs/05-api-spec.md` 에는 아직 없고, 코드(`app/routers/admin.py`)가 유일한 출처입니다.

**충돌은 원천 차단돼 있습니다.** 로그인은 같은 `POST /v1/admin/login` 이지만 스탑갭은
`api.nutti.co.kr` 에서 서빙되고 이 SPA 는 `play.nutti.co.kr` 이라 **저장소 자체가 분리**
됩니다(스탑갭은 `sessionStorage['nutti_admin_token']`, 우리는 `localStorage['nutti.session.*']`).
아래 1번이 W-11 의 전제로 요구한 「자기 토큰 저장소 + 자기 401 정책을 가진 별도 앱」을
스탑갭은 **오리진 분리로 공짜로 만족**한 셈입니다. 반대로 제가 이 SPA 안에 W-11 을 넣으면
그 요건은 여전히 제가 직접 지어야 하고, `assertNotAdminPath` 도 그대로 유효합니다.

FR-W11 대비 남은 공백은 셋입니다:

| FR | 스탑갭 | 남은 것 |
|---|---|---|
| 03 성과 지표 대시보드 | 스타일 표의 **3개 열** + 잡 로그·행동 이벤트 **원본 표**(PR #264) | 추이·기간 필터·차트 — 집계가 없음 |
| 06 스타일별 모델/해상도/시드 | **없음** — 아래 |
| 08 유입 지표 모니터링 | 행동 이벤트 원본 표(PR #264)로 `event_type` 필터만 | 유입원별 집계·기간 비교 |

**FR-W11-06 은 「미구현」이 아니라 부채를 쌓는 중입니다.** 프롬프트 버전 생성 API 는
`model_config` 를 받는데 스탑갭이 안 보내서 **콘솔로 만든 버전은 영영 `{}`** 이고, 수정
바디에는 그 필드가 없어 나중에 채울 방법도 없습니다. 운영이 이 콘솔로 버전을 쌓기 시작하면
데이터에 남습니다 — W-11 을 만들 때 이미 쌓인 `{}` 를 상대해야 한다는 뜻입니다.

덤: 아래 2번에서 함정으로 적어 둔 **`share_rate` 가 1 을 넘는 것**을 스탑갭이 그대로
밟습니다 — `(share_rate*100).toFixed(1)%` 로 찍어서 `100%+` 셀이 나옵니다.
...
- ~~비주얼 디자인이 없습니다.~~ **닫혔습니다**(PR #35·#36). 브랜드 색·서체가 어느 문서에도
  ...
  **OG/공유 이미지는 아직 없습니다.**
- ~~**W-01 히어로 이미지가 자리표시자입니다.**~~ **닫혔습니다**(2026-09-01). 도형이던
  `before.svg`·`after.svg` 는 **같은 강아지의 실제 원본/변환 한 쌍**으로 교체됐고,
  같은 날 **자사 사진 쌍**으로 다시 갈았습니다 — `public/hero/before.webp`(사용자가
  직접 찍은 본인 강아지)와 그 사진을 누띠에 돌려 나온 `after.webp`(하늘을 나는 모습).
  전에 쓰던 Pixabay 사진은 라이선스 문제가 아니라 자사 사진이 생겨서 내렸습니다.
  둘 다 **4:3 · 1072×804** — 히어로 프레임과 같은 비라 브라우저가 더 깎지 않습니다.
  라이선스 근거·크롭 좌표·교체 규칙은 `public/hero/NOTICE.md` — 특히 «원본과 결과는
  항상 쌍으로 교체» 항목을 읽고 바꾸세요(파생물도 같이 라이선스에 걸립니다).
- ~~**W-06 쇼핑몰 배너에 상품 썸네일이 없습니다**(FR-W06-06).~~ **닫혔습니다**(이슈 #319,
  2026-09-14). 쇼핑몰 메인 첫 자리의 포크스틱 상품 사진을 `public/shop/pork-stick.webp`
  (128×128, 3KB)로 — 출처·가공·교체 규칙은 `public/shop/NOTICE.md`. 상품명은 배너에 싣지
  않습니다(쇼핑몰 상품명의 「콜라겐·칼슘」류가 FR-W06-11 에 걸립니다).
...
4. **W-02는 전체를 한 페이지에** 받습니다(카탈로그 페이지네이션 없음 — 시드 기준
   40개, PR #95). 썸네일 `loading="lazy"`는 옵션이 아니라 필수입니다.
...
13. **스타일 입력은 만드는 입구가 둘입니다.** `input_fields` 를 가진 25종은 W-04 «이대로
