...
- ~~**W-01 히어로 이미지가 자리표시자입니다.**~~ **닫혔습니다**(2026-09-01). 도형이던
  `before.svg`·`after.svg` 는 **같은 강아지의 실제 원본/변환 한 쌍**으로 교체됐습니다 —
  `public/hero/before.webp`(Pixabay ID 1903313, 상업 이용 가능)와 그 사진을 누띠에
  돌려 나온 `after.webp`(«KONG-I» 패키지)입니다. 둘 다 3:2 · 1200×800 로 맞췄습니다.
  라이선스 근거·가공 내역·교체 규칙은 `public/hero/NOTICE.md` — 특히 «원본과 결과는
