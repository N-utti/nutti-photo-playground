...
### 1. admin 토큰은 사용자 세션과 상호 배타 — 「번들 분리」가 취향이 아니게 됐습니다
...
게스트 재발급 재시도도 없습니다. **지금의 공용 클라이언트로 콘솔을 한 번 부르면 보고 있던
사용자가 로그아웃됩니다.**
...
아니라 토큰 저장소와 401 정책이 다른 두 앱이라는 뜻**입니다. 콘솔은 자기 클라이언트를 갖고,
401 이면 사용자 세션을 건드리지 말고 관리자 로그인 화면으로만 돌아가야 합니다.
