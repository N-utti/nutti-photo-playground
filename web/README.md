...
2026-08-26 기준 남은 건 `events` · `admin` 둘뿐입니다. **표가 «501» 이라고 적혀 있다는
이유로 목을 계속 믿지 마세요**: 공유는 08-12 이전, 계산기 연결은 08-19 에 구현됐는데 이
표는 08-25 까지 둘 다 501 이라고 말하고 있었습니다.
...
| Phase 6 | GA4 크로스도메인 · UTM · 이벤트 비콘 | **프론트 완료 · 관리자 설정 대기** — 아래 |
...
**아직 안 켜졌습니다 — 남은 건 코드가 아닙니다.**

1. **GA4 관리자 → 데이터 스트림 → 태그 설정 → 도메인 구성**에 놀이터 오리진을 등록해야
   합니다. GA4 크로스도메인은 관리자 UI 에서만 설정하고(코드의 `linker` 파라미터는 UA
   시절 방식), 등록되면 gtag 가 그 도메인으로 나가는 링크에 `_gl` 을 자동으로 붙입니다.
2. **놀이터 도메인이 미정입니다.** `docs/06` §11 이 「예: `play.nutti.co.kr`」이라고만
   적어 뒀습니다 — 1번에 등록할 값이 이것입니다.

확인 방법: 놀이터에서 쇼핑몰·계산기 링크를 눌러 도착한 주소에 **`_gl=` 이 붙는지** 봅니다.
